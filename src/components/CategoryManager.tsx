...
export function CategoryManager() {
  ...
  async function submit() {
    ...
    if (editing) {
      ...
      await addCategory({ name: trimmed, type, icon: "circle-dot", color, isActive: true });
    }
    setName("");
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3 rounded-xl border border-line bg-surface p-4">
        <div className="grid gap-3 sm:grid-cols-[1fr_140px_auto]">
          <Field
            label={editing ? t("category.editCategory") : t("category.newCategory")}
          ...
          <div className="flex items-end gap-2">
            <Button onClick={() => void submit()} type="button" variant="primary">
        ...
        {/* Color swatch row */}
        <div>
          <p className="mb-2 text-xs font-medium text-muted">{t("category.color")}</p>
        ...
        {(["expense", "income"] as const).map((categoryType) => (
          <div className="panel p-4" key={categoryType}>
            <h3 className="text-sm font-semibold capitalize text-ink">
              {categoryType === "expense" ? t("category.expense") : t("category.income")}
            </h3>
                    ...
                    <div className="flex min-w-0 items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 flex gap-2">
                          ...
                          {!category.isActive ? (
                            <Badge tone="warning">{t("category.disabled")}</Badge>
                          ) : null}
                        </div>
                      </div>
                    </div>
